...
int main(){
    ...
    while(1){
        ...
        //position
            ...
            pid.target(26.02298429,8.136726923,0);//ambil ke3
            caseBot = 0;
        }else if(caseBot == 6){
            pid.target(29.9841152,8.5217,0);//ambil ke 4
            for(int i = 0;i<1000;i++){
                atas.kirimData(sockfd, std::to_string(R1));
            }
            wait(2);
            ...
            atas.kirimData(sockfd, std::to_string(R1));
            pid.target(31.185461,24.27863633,0);//tanam ke3
            caseBot =0;
        }else if(caseBot == 8){
            pid.target(29.72854578,18.63473189,0);//tanam  k4
            for(int i = 0;i<1000;i++){
                atas.kirimData(sockfd, std::to_string(L1));
            }
            caseBot =0;
            ...
            pid.target(39.2831055,7.259113636,0);//ambil ke 5
            caseBot =0;
            
        }else if(caseBot == 10){
            pid.target(41.995319,8.71562,0);//ambil ke 6
            for(int i = 0; i<1000;i++){
                atas.kirimData(sockfd, std::to_string(R1));
            }
            wait(2);
            ...
            for(int i = 0; i<1000;i++){
                atas.kirimData(sockfd, std::to_string(R1));
            }pid.target(41.48121778,26.22522322,0);//tanam ke 5
            caseBot =0;
        }else if(caseBot == 12){
            pid.target(41.2932358,20.653398,0);//tanam ke 6
            for(int i =0;i<1000;i++){
                atas.kirimData(sockfd, std::to_string(L1));
            }caseBot =0;
        }else if(caseBot == 13){
