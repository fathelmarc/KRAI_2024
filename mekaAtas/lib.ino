...
void motorWorm(int pwmval) {
  pwmval = max(-255, min(pwmval, 255));
  analogWrite(pwmWorm, fabs(pwmval));

  if (pwmval < 0) {
    digitalWrite(cwWorm, 1);
    digitalWrite(ccwWorm, 0);
  }
  else if (pwmval > 0) {
    digitalWrite(cwWorm, 0);
    digitalWrite(ccwWorm, 1);
  }
  else {
    digitalWrite(cwWorm, 0);
    digitalWrite(ccwWorm, 0);
